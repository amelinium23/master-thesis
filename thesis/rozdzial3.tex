...
W celu zbadania wydajności danego środowiska uruchomieniowego, skonstruowano odpowiednie eksperymenty, które sprawdzają wydajność algorytmu sortowania. Wszystkie algorytmy sortowania zostały przetestowane dla każdego środowiska.
...
W tabeli \ref{tab:sorting_experiments} przedstawiono liczbę iteracji oraz liczbę elementów dla przeprowadzonych eksperymentów.
...
\begin{figure}[H]
  ...
  \end{subfigure}
  \hfill
  \caption{Wyniki eksperymentów dla algorytmu sortowania bąbelkowego dla 100 iteracji i 1000 elementów - a) czas wykonania jednorazowego testu w milisekundach, b) ilość zajmowanej pamięci w kilobajtach (kB) Źródło: opracowanie własne}
  \label{fig:bubble_sorting_e1}
\end{figure}

Na wykresie \ref{fig:bubble_sorting_e1_time} zauważono, że czas wykonania w przypadku pierwszych 10 iteracji jest zwiększony w porównaniu do pozostałych próbek. Najprawdopodobniej jest to spowodowane tzw. rozgrzewaniem się środowiska, jest to zauważalne w przypadku środowiska NodeJS. Natomiast, z wykresu \ref{fig:bubble_sorting_e1_memory} zaobserwowano tendencję wzrostową zużycia pamięci operacyjnej, największym zużyciem pamięci operacyjnej deklaruje środowisko Bun, które w ostatniej iteracji zużywa najwięcej ze wszystkich środowisk uruchomieniowych. 
...
\begin{figure}[H]
  ...
  \end{subfigure}
  \caption{Wyniki eksperymentów dla algorytmu sortowania bąbelkowego dla 1000 iteracji i 1000 elementów dla języka JavaScript - a) czas wykonania jednorazowego testu w milisekundach, b) ilość zajmowanej pamięci w kilobajtach (kB) Źródło: opracowanie własne}
  \label{fig:bubble_sorting_e2}
\end{figure}

Przy zwiększonej liczbie próbek zauważono na wykresie \ref{fig:bubble_sorting_e2_time}, że czas wykonania jest podobny względem wszystkich środowiska uruchomieniowych, Bun charakteryzuje się niższym czasem wykonania w porównaniu do pozostałych środowisk. Natomiast z wykresu \ref{fig:bubble_sorting_e2_memory} zaobserwowano, że Bun charakteryzuje się dużym zużyciem pamięci operacyjnej w porównaniu do pozostałych środowisk.
...
\begin{figure}[H]
  ...
  \end{subfigure}
  \caption{Wyniki eksperymentów dla algorytmu sortowania bąbelkowego dla 1000 iteracji i 1000 elementów dla języka TypeScript - a) czas wykonania jednorazowego testu w milisekundach, b) ilość zajmowanej pamięci w kilobajtach (kB) Źródło: opracowanie własne}
  \label{fig:bubble_sorting_e2_ts}
\end{figure}

W przypadku zwiększonej liczbie próbek, na wykresie \ref{fig:bubble_sorting_e2_ts_time} zaobserwowano, że czas wykonania wzrósł w porównaniu do poprzednich próbek. Wraz z zwiększeniem liczbie próbek zwiększyło się zapotrzebowanie na pamięć operacyjną, co jest zauważalne na wykresach \ref{fig:bubble_sorting_e2_ts_memory}. Takie zachowanie środowisk może być spowodowane nieodpowiednim zarządzaniem przez środowiska pamięcią operacyjną.
...
\begin{figure}[H]
  ...
  \end{subfigure}
  \caption{Wyniki eksperymentów dla algorytmu sortowania bąbelkowego dla 1000 iteracji i 10000 elementów dla języka JavaScript - a) czas wykonania jednorazowego testu w milisekundach, b) ilość zajmowanej pamięci w kilobajtach (kB) Źródło: opracowanie własne}
  \label{fig:bubble_sorting_e3}
\end{figure}

Na wykresie \ref{fig:bubble_sorting_e3_time} zauważono, że czas wykonania w przypadku pierwszych 10 iteracji jest zwiększony w porównaniu do pozostałych próbek. Natomiast z wykresu \ref{fig:bubble_sorting_e3_memory} zaobserwowano tendencję wzrostową zużycia pamięci operacyjnej, największym zużyciem pamięci operacyjnej deklaruje środowisko Bun, które w ostatniej iteracji zużywa najwięcej ze wszystkich środowisk uruchomieniowych. Momenty, w których zużycie pamięci spada może być spowodowane zwolnieniem pamięci operacyjnej przez środowisko za pomocą mechanizmu Garbage Collector.
...
\begin{figure}[H]
  ...
  \end{subfigure}
  \caption{Wyniki eksperymentów dla algorytmu sortowania bąbelkowego dla 1000 iteracji i 10000 elementów dla języka TypeScript - a) czas wykonania jednorazowego testu w milisekundach, b) ilość zajmowanej pamięci w kilobajtach (kB) Źródło: opracowanie własne}
  \label{fig:bubble_sorting_e3_ts}
\end{figure}

Na wykresie \ref{fig:bubble_sorting_e3_ts_time} zauważono, że czas wykonania w przypadku pierwszych 10 iteracji jest zwiększony w porównaniu do pozostałych próbek. Natomiast, z wykresu \ref{fig:bubble_sorting_e3_ts_memory} zaobserwowano tendencję wzrostową zużycia pamięci operacyjnej, największym zużyciem pamięci operacyjnej deklaruje środowisko Bun, które w ostatniej iteracji zużywa najwięcej ze wszystkich środowisk uruchomieniowych.
...
Na wykresie \ref{fig:bubble_sorting_e4_time} zauważono, że czas wykonania w przypadku pierwszych iteracji jest zwiększony w porównaniu do pozostałych próbek. Zauważono także, że środowisko NodeJS ma najdłuższy czas wykonania algorytmu w pierwszych iteracjach. Natomiast z wykresu \ref{fig:bubble_sorting_e4_memory} zaobserwowano tendencję wzrostową zużycia pamięci operacyjnej, największym zużyciem pamięci operacyjnej deklaruje środowisko Bun, które w ostatniej iteracji zużywa najwięcej ze wszystkich środowisk uruchomieniowych.
...
Na wykresie \ref{fig:bubble_sorting_e4_ts_time} zauważono, że czas wykonania w przypadku pierwszych iteracji jest zwiększony w porównaniu do pozostałych próbek. Zauważono także, że środowisko NodeJS ma najdłuższy czas wykonania algorytmu w pierwszych iteracjach. Natomiast z wykresu \ref{fig:bubble_sorting_e4_ts_memory} zaobserwowano tendencję wzrostową zużycia pamięci operacyjnej, największym zużyciem pamięci operacyjnej deklaruje środowisko Bun, które w ostatniej iteracji zużywa najwięcej ze wszystkich środowisk uruchomieniowych.
...
\begin{figure}[H]
  ...
  \end{subfigure}
  \caption{Wyniki eksperymentów dla algorytmu sortowania szybkiego dla 100 iteracji i 1000 elementów - a) czas wykonania jednorazowego testu w milisekundach, b) ilość zajmowanej pamięci w kilobajtach (kB) Źródło: opracowanie własne}
  \label{fig:quick_sorting_e1}
\end{figure}

Na wykresie \ref{fig:quick_sorting_e1_time} zaobserwowano, że czas wykonania w przypadku pierwszych 10 iteracji jest zwiększony w porównaniu do pozostałych próbek. Natomiast, z wykresu \ref{fig:quick_sorting_e1_memory} zauważono tendencję wzrostową zużycia pamięci operacyjnej, największym zużyciem pamięci operacyjnej deklaruje środowisko Bun, które w ostatniej iteracji zużywa najwięcej ze wszystkich środowisk uruchomieniowych.
...
Na rysunku \ref{fig:quick_sorting_e1_time} zauważono, że czas wykonania w przypadku pierwszych 10 iteracji jest zwiększony w porównaniu do pozostałych próbek. Natomiast, z wykresu \ref{fig:quick_sorting_e1_memory} zaobserwowano tendencję wzrostową zużycia pamięci operacyjnej, największym zużyciem pamięci operacyjnej deklaruje środowisko Bun, które w ostatniej iteracji zużywa najwięcej ze wszystkich środowisk uruchomieniowych.
...
Na rysunku \ref{fig:quick_sorting_e2_time} zauważono, że czas wykonania w przypadku pierwszych 10 iteracji jest zwiększony w porównaniu do pozostałych próbek. Natomiast, z wykresu \ref{fig:quick_sorting_e2_memory} zaobserwowano tendencję wzrostową zużycia pamięci operacyjnej, największym zużyciem pamięci operacyjnej deklaruje środowisko Bun, które w ostatniej iteracji zużywa najwięcej ze wszystkich środowisk uruchomieniowych.
...
\begin{figure}[H]
  ...
  \end{subfigure}
  \caption{Wyniki eksperymentów dla algorytmu sortowania szybkiego dla 1000 iteracji i 1000 elementów - a) czas wykonania jednorazowego testu w milisekundach, b) ilość zajmowanej pamięci w kilobajtach (kB) Źródło: opracowanie własne}
...
\begin{figure}[H]
  ...
  \end{subfigure}
  \caption{Wyniki eksperymentów dla algorytmu sortowania szybkiego dla 100 iteracji i 10000 elementów - a) czas wykonania jednorazowego testu w milisekundach, b) ilość zajmowanej pamięci w kilobajtach (kB) Źródło: opracowanie własne}
  \label{fig:quick_sorting_e3}
\end{figure}

Na rysunku \ref{fig:quick_sorting_e3_time} zauważono, iż NodeJS posiadał najdłuższe czasy wykonywania sortowania w pierwszych iteracjach, natomiast największe rozbieżności w czasach wykonania posiadało środowisko Bun. Z wykresu \ref{fig:quick_sorting_e3_memory} zauważono, że środowisko Bun zużywało najwięcej pamięci operacyjnej, natomiast środowisko Deno zużywało najmniej pamięci operacyjnej.
...
\begin{figure}[H]
  ...
  \end{subfigure}
  \caption{Wyniki eksperymentów dla algorytmu sortowania szybkiego dla 1000 iteracji i 10000 elementów - a) czas wykonania jednorazowego testu w milisekundach, b) ilość zajmowanej pamięci w kilobajtach (kB) Źródło: opracowanie własne}
...
\begin{figure}[H]
  ...
  \end{subfigure}
  \caption{Wyniki eksperymentów dla algorytmu sortowania pozycyjnego dla 100 iteracji i 1000 elementów - a) czas wykonania jednorazowego testu w milisekundach, b) ilość zajmowanej pamięci w kilobajtach (kB) Źródło: opracowanie własne}
  \label{fig:radix_sorting_e1}
\end{figure}

Na wykresie \ref{fig:radix_sorting_e1_time} zauważono, że w pierwszych iteracjach środowisko NodeJs wykonuje najdłużej sortowanie, natomiast Bun posiada duże zróżnicowanie czasu wykonania testu, w pozostałych iteracjach możemy zobaczyć zbliżone czasy wykonania dla NodeJS oraz Deno. Z wykresu \ref{fig:radix_sorting_e1_memory} zaobserwowano, że środowisko Bun zużywa najwięcej pamięci operacyjnej, natomiast środowisko Deno zużywa najmniej pamięci operacyjnej.
...
\begin{figure}[H]
  ...
  \end{subfigure}
  \caption{Wyniki eksperymentów dla algorytmu sortowania pozycyjnego dla 1000 iteracji i 1000 elementów - a) czas wykonania jednorazowego testu w milisekundach, b) ilość zajmowanej pamięci w kilobajtach (kB) Źródło: opracowanie własne}
...
\begin{figure}[H]
  ...
  \end{subfigure}
  \caption{Wyniki eksperymentów dla algorytmu sortowania pozycyjnego dla 100 iteracji i 10000 elementów - a) czas wykonania jednorazowego testu w milisekundach, b) ilość zajmowanej pamięci w kilobajtach (kB) Źródło: opracowanie własne}
  \label{fig:radix_sorting_e3}
\end{figure}

Dla zwiększonej liczby elementów w tablicy na rysunku \ref{fig:radix_sorting_e3_time} zaobserwowano, że środowisko NodeJS w pierwszych iteracjach posiada najdłuższe czasy wykonania sortowania. Na rysunku \ref{fig:radix_sorting_e3_memory} zaobserwowano, że środowisko Bun zużywa najwięcej pamięci operacyjnej, natomiast środowisko NodeJS zużywa najmniej pamięci operacyjnej.
...
\begin{figure}[H]
  ...
  \end{subfigure}
  \hfill
  \caption{Wyniki eksperymentów dla algorytmu sortowania pozycyjnego dla 1000 iteracji i 10000 elementów - a) czas wykonania jednorazowego testu w milisekundach, b) ilość zajmowanej pamięci w kilobajtach (kB) Źródło: opracowanie własne}
...
\begin{table}[H]
  ...
  \par\vspace{0.3cm} 
  \parbox{0.9\linewidth}{
    \centering Źródło: \cite{cryptoeprint:2022/361}
  }
...
\begin{figure}[H]
  ...
  \end{subfigure}
  \hfill
  \caption{Wyniki eksperymentów dla operacji dekodowania z wykorzystaniem algorytmu \textit{Base64} dla 100 iteracji oraz 32768 znaków - a) czas wykonania jednorazowego testu w milisekundach, b) ilość zajmowanej pamięci w kilobajtach (kB) Źródło: opracowanie własne}
  \label{fig:decoding_e1_js}
\end{figure}

Na rysunku \ref{fig:decoding_e1_js_time} zaobserwowano, że środowisko NodeJs posiada najkrótsze czasy wykonania operacji dekodowania, natomiast najdłuższe czasy posiada środowisko Deno. Na rysunku \ref{fig:decoding_e1_js_memory} zaobserwowano, że środowisko Bun zużywa najwięcej pamięci operacyjnej, natomiast środowisko NodeJS zużywa najmniej pamięci operacyjnej.
...
\begin{figure}[H]
  ...
  \end{subfigure}
  \hfill
  \caption{Wyniki eksperymentów dla operacji kodowania z wykorzystaniem algorytmu \textit{Base64} dla 100 iteracji oraz 32768 znaków - a) czas wykonania jednorazowego testu w milisekundach, b) ilość zajmowanej pamięci w kilobajtach (kB) Źródło: opracowanie własne}
  \label{fig:encoding_e1_ts}
\end{figure}

Na rysunku \ref{fig:encoding_e1_ts_time} zaobserwowano, że środowisko NodeJS oraz Bun posiadają zbliżone czasy wykonania operacji kodowania, natomiast najdłuższe czasy posiada środowisko Deno. Na rysunku \ref{fig:encoding_e1_ts_memory} zaobserwowano, że środowisko Bun zużywa najwięcej pamięci operacyjnej, natomiast środowisko NodeJS zużywa najmniej pamięci operacyjnej.
...
Na rysunku \ref{fig:decoding_e1_ts_time} zaobserwowano, że środowisko NodeJS posiada najkrótsze czasy wykonania operacji dekodowania, natomiast najdłuższe czasy posiada środowisko Deno. Na rysunku \ref{fig:decoding_e1_ts_memory} zaobserwowano, że środowisko Bun zużywa najwięcej pamięci operacyjnej, natomiast środowisko NodeJS zużywa najmniej pamięci operacyjnej.
...
\begin{figure}[H]
  ...
  \end{subfigure}
  \hfill
  \caption{Wyniki eksperymentów dla operacji kodowania z wykorzystaniem algorytmu \textit{Base64} dla 1000 iteracji oraz 8192 znaków - a) czas wykonania jednorazowego testu w milisekundach, b) ilość zajmowanej pamięci w kilobajtach (kB) Źródło: opracowanie własne}
  \label{fig:encoding_e2_js}
\end{figure}

Na rysunku \ref{fig:encoding_e2_js_time} zaobserwowano, że środowiska NodeJS oraz Bun posiadają podobne czasy kodowania, natomiast najdłuższe czasy posiada środowisko Deno. Na rysunku \ref{fig:encoding_e2_js_memory} zaobserwowano, że środowisko Bun zużywa najwięcej pamięci operacyjnej, natomiast środowisko NodeJS zużywa najmniej pamięci operacyjnej.
...
\begin{figure}[H]
  ...
  \end{subfigure}
  \hfill
  \caption{Wyniki eksperymentów dla operacji dekodowania z wykorzystaniem algorytmu \textit{Base64} dla 1000 iteracji oraz 8192 znaków - a) czas wykonania jednorazowego testu w milisekundach, b) ilość zajmowanej pamięci w kilobajtach (kB) Źródło: opracowanie własne}
  \label{fig:decoding_e2_js}
\end{figure}

Na rysunku \ref{fig:decoding_e2_js_time} zaobserwowano, że środowisko NodeJS posiada najkrótsze czasy wykonania operacji dekodowania, natomiast najdłuższe czasy posiada środowisko Deno. Na rysunku \ref{fig:decoding_e2_js_memory} zaobserwowano, że środowisko Bun zużywa najwięcej pamięci operacyjnej, natomiast środowisko NodeJS zużywa najmniej pamięci operacyjnej.
...
Na rysunku \ref{fig:decoding_e2_ts_time} zaobserwowano, że środowisko NodeJS posiada najkrótsze czasy wykonania operacji dekodowania, natomiast najdłuższe czasy posiada środowisko Deno. Na rysunku \ref{fig:decoding_e2_ts_memory} zaobserwowano, że środowisko Bun zużywa najwięcej pamięci operacyjnej, natomiast środowisko NodeJS zużywa najmniej pamięci operacyjnej.
...
\begin{table}[H]
  ...
  \begin{tabular}{|c|c|c|}
    \hline
    \textbf{Liczba eksperymentów} & \textbf{Rozmiar pliku} & \textbf{Liczba plików} \\ \hline
    50 & 512kB & 50 \\ \hline
    50 & 1MB & 50 \\ \hline
  \end{tabular}
  \par\vspace{0.3cm} 
  \parbox{0.9\linewidth}{
    \centering Źródło: opracowanie własne
  }
...
\begin{figure}[H]
  ...
  \end{subfigure}
  \hfill
  \caption{Wynik eksperymentów odczytu pliku dla 50 iteracji oraz 50 plików o rozmiarze 512kB - a) czas wykonania jednorazowego testu w milisekundach, b) ilość zajmowanej pamięci w kilobajtach (kB) Źródło: opracowanie własne}
...
\begin{figure}[H]
  ...
  \end{subfigure}
  \hfill
  \caption{Wynik eksperymentów zapisu pliku dla 50 iteracji oraz 50 plików o rozmiarze 512kB - a) czas wykonania jednorazowego testu w milisekundach, b) ilość zajmowanej pamięci w kilobajtach (kB) Źródło: opracowanie własne}
...
\begin{figure}[H]
  ...
  \end{subfigure}
  \hfill
  \caption{Wynik eksperymentów odczytu pliku dla 50 iteracji oraz 50 plików o rozmiarze 1MB - a) czas wykonania jednorazowego testu w milisekundach, b) ilość zajmowanej pamięci w kilobajtach (kB) Źródło: opracowanie własne}
...
\begin{figure}[H]
  ...
  \end{subfigure}
  \hfill
  \caption{Wynik eksperymentów zapisu pliku dla 50 iteracji oraz 50 plików o rozmiarze 1MB - a) czas wykonania jednorazowego testu w milisekundach, b) ilość zajmowanej pamięci w kilobajtach (kB) Źródło: opracowanie własne}
...
\begin{table}[H]
  ...
  \par\vspace{0.3cm} 
  \parbox{0.9\linewidth}{
    \centering Źródło: \cite{NodeAndBun}
  }
...
\begin{figure}[H]
  ...
  \end{subfigure}
  \hfill
  \caption{Wynik eksperymentów dla testu obciążeniowego serwera HTTP 100 równoczesnych połączeń oraz 10000 żądań HTTP, wyrażonego w RPS (\textit{Request per second}) - a) dla języka JavaScript, b) dla języka TypeScript Źródło: opracowanie własne}
  \label{fig:server_e1}
\end{figure}

Zarówno dla języka JavaScript jak i języka TypeScript, rysunek \ref{fig:server_e1} zaobserwowano, że środowisko Deno posiada najwyższe wyniki testu obciążeniowego wyrażonego w RPS (\textit{Request per second}), natomiast środowisko Bun posiada najniższe wyniki testu obciążeniowego wyrażonego w RPS (\textit{Request per second}).
...
\begin{figure}[H]
  ...
  \end{subfigure}
  \hfill
  \caption{Wynik eksperymentów dla testu obciążeniowego serwera HTTP 1000 równoczesnych połączeń oraz 10000 żądań HTTP, wyrażonego w RPS (\textit{Request per second}) - a) dla języka JavaScript, b) dla języka TypeScript Źródło: opracowanie własne}
  \label{fig:server_e2}
\end{figure}

Zarówno dla języka JavaScript jak i języka TypeScript, rysunek \ref{fig:server_e2} zaobserwowano, że środowisko Deno posiada najwyższe wyniki testu obciążeniowego wyrażonego w RPS (\textit{Request per second}), natomiast środowisko Bun posiada najniższe wyniki testu obciążeniowego wyrażonego w RPS (\textit{Request per second}).
...
\begin{figure}[H]
  ...
  \end{subfigure}
  \hfill
  \caption{Wynik eksperymentów dla testu obciążeniowego serwera HTTP 10000 równoczesnych połączeń oraz 10000 żądań HTTP, wyrażonego w RPS (\textit{Request per second}) - a) dla języka JavaScript, b) dla języka TypeScript Źródło: opracowanie własne}
  \label{fig:server_e3}
\end{figure}

Zarówno dla języka JavaScript jak i języka TypeScript, rysunek \ref{fig:server_e3} zaobserwowano, że środowisko Deno posiada najwyższe wyniki testu obciążeniowego wyrażonego w RPS (\textit{Request per second}), natomiast środowisko Bun posiada najniższe wyniki testu obciążeniowego wyrażonego w RPS (\textit{Request per second}).
...
\begin{table}[H]
  ...
  \par\vspace{0.3cm} 
  \parbox{0.9\linewidth}{
    \centering Źródło: \cite{sqlite_performance}
  }
...
\begin{figure}[H]
  ...
  \end{subfigure}
  \caption{Wyniki eksperymentów dla testu bazy danych 10 iteracji oraz 1000 rekordów - po lewej czas wykonania jednorazowego testu w milisekundach, po prawej ilość zajmowanej pamięci w kilobajtach (kB) Źródło: opracowanie własne}
...
\begin{figure}[H]
  ...
  \end{subfigure}
  \caption{Wyniki eksperymentów dla testu bazy danych 100 iteracji oraz 1000 rekordów - po lewej czas wykonania jednorazowego testu w milisekundach, po prawej ilość zajmowanej pamięci w kilobajtach (kB) Źródło: opracowanie własne}
